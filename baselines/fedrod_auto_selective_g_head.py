...
class CustomLinearLayers(nn.Module):
    def __init__(self, in_features, out_features, num_layers=10, device="cpu"):
        super(CustomLinearLayers, self).__init__()
        self.layers = nn.ModuleList([nn.Linear(in_features, out_features) for _ in range(num_layers)])
        self.weights = nn.Parameter(torch.rand(num_layers))  # learnable weights
        self.device = device
        for layer in self.layers:
            nn.init.sparse_(layer.weight, sparsity=0.6)
            layer.to(device)
        self.weights.to(device)

    def forward(self, x):
        # Apply each linear layer, multiply by its weight, and sum up
        output = sum(w * layer(x) for w, layer in zip(self.weights, self.layers))
        return output
...
if __name__ == '__main__':
    ...
    g_head = CustomLinearLayers(in_features, out_features, num_layers=args.num_users, device=args.device)
    ...
    for rnd in tqdm(range(args.rounds)):
        ...
        for client_id in range(args.num_users):  # training over the subset, in fedper, all clients train
            ...
            w_locals[client_id], g_aux_temp, g_head_temp, l_heads[client_id], loss_local = local.update_weights_auto_selective_ghead(net=copy.deepcopy(model).to(args.device), g_head = copy.deepcopy(g_head).to(args.device), g_aux = copy.deepcopy(g_aux).to(args.device), l_head = l_heads[client_id], epoch=args.local_ep, loss_switch = loss_switch)
            g_auxs.append(g_aux_temp)
            g_heads.append(g_head_temp)
        ...
        if aggregation_switch == 'fedavg':
            ...
            g_head = FedAvg_noniid_classifier(g_heads, dict_len)
